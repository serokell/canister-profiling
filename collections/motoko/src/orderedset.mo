...
actor {
    ...
    let natSet = Set.Make<Nat64>(Nat64.compare);
    stable var rbSet = natSet.empty();
    let rand = Random.new(null, 42);

    public func generate(size: Nat32) : async () {
        let rand = Random.new(?size, 1);
        rbSet := natSet.fromIter(rand);
    };
    ...
    public func batch_get(n : Nat) : async () {
        for (_ in Iter.range(1, n)) {
            ignore natSet.contains(rbSet, Option.get<Nat64>(rand.next(), 0));
        }
    };
    public func batch_put(n : Nat) : async () {
        for (_ in Iter.range(1, n)) {
            let k = Option.get<Nat64>(rand.next(), 0);
            rbSet := natSet.put(rbSet, k);
        }
    };
    public func batch_remove(n : Nat) : async () {
        let rand = Random.new(null, 1);
        for (_ in Iter.range(1, n)) {
            rbSet := natSet.delete(rbSet, Option.get<Nat64>(rand.next(), 0));
        }
    };

    public func intersect(n : Nat32) : async () {
      let rand = Random.new (?n, 1);
      let set2 = natSet.fromIter(rand);
      ignore natSet.intersect(rbSet, set2)
    };

    public func union(n : Nat32) : async () {
      let rand = Random.new (?n, 1);
      let set2 = natSet.fromIter(rand);
      ignore natSet.union(rbSet, set2)
    };

    public func diff(n : Nat32) : async () {
      let rand = Random.new (?n, 1);
      let set2 = natSet.fromIter(rand);
      ignore natSet.diff(rbSet, set2)
    };

    public func equals(n : Nat32) : async () {
      ignore natSet.equals(rbSet, rbSet)
    };

    public func isSubset(n : Nat32) : async () {
      ignore natSet.isSubset(rbSet, rbSet)
    };

    public func foldLeft() : async () {
        ignore natSet.foldLeft<Nat64>(rbSet, 0, func (x, acc) {x + acc});
    };
    public func foldRight() : async () {
        ignore natSet.foldRight<Nat64>(rbSet, 0, func (x, acc) {x + acc});
    };
    public func mapfilter() : async () {
        ignore natSet.mapFilter<Nat64>(rbSet, func (elem) {
          switch (elem % 2) {
            case 1 { null };
            case _ { ?elem }
          };
    ...
    public func map() : async () {
      ignore natSet.map<Nat64>(rbSet, func (elem) {elem + elem})
    };
}
